...
import {
  GetProfesorPrimariaSuccessResponse,
  GetProfesoresPrimariaSuccessResponse,
} from "../../../interfaces/shared/apis/api01/profesores-primaria/types";
import { buscarProfesoresPrimariaConFiltros } from "../../../../core/databases/queries/RDP02/profesor-primaria/buscarProfesoresPrimariaConFiltros";
import { contarProfesoresPrimariaConFiltros } from "../../../../core/databases/queries/RDP02/profesor-primaria/contarProfesoresPrimariaConFiltros";
import { obtenerDetallesDeProfesorPrimariaPorId } from "../../../../core/databases/queries/RDP02/profesor-primaria/obtenerDetalleDeProfesorPrimariaPorId";
...
// Listar / buscar profesores de primaria con filtros
router.get("/", (async (req: Request, res: Response) => {
  try {
    ...
    // Parseo de Aula independiente de sinAulaBool (evita ignorar filtros silenciosamente)
    ...
    const [totalResultados, profesores] = await Promise.all([
      contarProfesoresPrimariaConFiltros(filtros, rdp02EnUso),
      buscarProfesoresPrimariaConFiltros(
        filtros,
        rdp02EnUso,
        numeroPagina,
    ...
    return res.status(200).json({
      success: true,
      message: "Profesores de primaria obtenidos exitosamente",
      data: profesores,
      paginacion: {
    ...
    } as GetProfesoresPrimariaSuccessResponse);
  } catch (error) {
    console.error("Error al obtener profesores de primaria:", error);
    ...
    return res.status(500).json({
      success: false,
      message: "Error al obtener profesores de primaria",
...
// Detalle de un profesor de primaria
router.get("/:idProfesorPrimaria", (async (req: Request, res: Response) => {
  try {
    const { idProfesorPrimaria } = req.params;
    const rdp02EnUso = req.RDP02_INSTANCE!;
    ...
    const profesor = await obtenerDetallesDeProfesorPrimariaPorId(
      idProfesorPrimaria,
      rdp02EnUso,
    );
    ...
    return res.status(200).json({
      success: true,
      message: "Profesor de primaria obtenido exitosamente",
      data: profesor,
    } as GetProfesorPrimariaSuccessResponse);
  } catch (error) {
    console.error("Error al obtener profesor de primaria:", error);
    ...
    return res.status(500).json({
      success: false,
      message: "Error al obtener profesor de primaria",
      errorType: SystemErrorTypes.UNKNOWN_ERROR,
      details: error,
    } as ErrorResponseAPIBase);
  }
